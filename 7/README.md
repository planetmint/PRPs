...
All machines were provided with the operating system *Ubuntu 18.04.1 LTS (Bionic Beaver)*. For simplicity we provided all machines (both the machine generating the workload and the machines running the Planetmint nodes) with the same software:
...
Planetmint was able to process 1,000,000 transactions in 56 minutes without any failure.
...
Name: d_commit, dtype: float64
```
Planetmint was able to process 1,000,000 transactions in about 26 minutes without any failure.
...
Planetmint was able to process 16,000 transactions in 18 seconds without any failure.
